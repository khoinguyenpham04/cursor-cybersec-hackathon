...
Last reviewed: 2026-08-01 (post PR #6 merge).
...
| 3 | **Campaign** | Overview/Cases → **Investigate sequence** → `fixture-boiling-frog` → score/actions | Mostly |
...
4. **Investigate sequence** → campaign case auto-kickoff → wait for
   `submit_campaign` → walk score, trail (412→419→430), recommended actions.
...
- [ ] **Surface fixture caveat** clearly on the campaign page: investigation
      uses ledger fixture facts, not the open GitHub repo’s live PR history
      (banner exists in spirit — make it unmistakable for judges).
...
- [ ] **Campaign sequence visualization** — timeline of PRs 412 → 419 → 430 with
      capability deltas from `agent/src/ledger/fixtures/boiling-frog.json`
      beside the score (today: result card + transcript only).
